...
function load(data){
	...
	for(var i = 0; i < parseInt(len); i++){
		...
		$("#fillblank").append("<tr><td>"+id+"</td>"+ course + chapter + "<td>"+level+"</td>"+ title + "<td>"+fixed+"</td>" + answer +
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(resCode == "000000"){
				...
				$("#fillblank").append("<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td>" +
						 "<td><a class=\"btn btn-success table_input_add\" href=\"javascript:void();\"><i class=\"fa fa-search-plus\"></i></a>"+  //添加
					     "</td></tr>");
...
function table_modify_confirm(){
	...
	var fixed = $("#table_input").children().find("input").parent().next().html();
	var ans;
	if(fixed == "判断题"){
		fixed = "3001";
		ans = $("#table_input").children().find("td").last().find("select").val();
	}else{
		fixed = "3000";
		ans = $("#table_input").children().find("td").last().find("input").val();
	}
	$.ajax({
		type : "post",
		url : "../fillblank/modifyfillblank",
		data:{"id":id,"title":title,"fixed":fixed,"ans":ans},
		success : function(data) {
			var resCode = eval("("+data+")").resCode;
			var resMsg = eval("("+data+")").resMsg;
			if(resCode == "000000"){
...
/* 因题型改变答案类型 */
$(document).on("change",".title_style",function(e){
	var xx = $(this).val();
	var x = $(this).parent().next();
	if(xx == "3001"){
		x.html("<select class=\"form-control\"><option value=\"\">请选择答案</option><option value=\"6000\">正确</option><option value=\"6001\">错误</option></select>");
	}
	e.stopPropagation();
});
...
$(document).on("click",".table_input_add",function(e){
	...
	$("#table_input").append("<table class=\"table table-striped table-bordered bootstrap-datatable datatable\">"+
			...
			"<th width=\"8%\">题号</th>"+
			"<th width=\"13%\">科目</th><th width=\"13%\">章节</th><th width=\"10%\">难度等级</th>"+
			"<th>题目</th><th width=\"10%\">题型</th>"+
			"<th width=\"20%\">答案</th></tr></thead>"+
			...
			"<td><select  class=\"form-control\"><option value=\"\">请选择</option><option value=\""+levelEasy+"\">简单</option><option value=\""+levelNormal+"\">一般</option><option value=\""+levelHard+"\">难</option></select></td>"+
			"<td><input class=\"form-control\"></td>" +
			"<td><select class=\"form-control title_style\"><option value=\"\">请选择</option><option value=\"3000\">填空题</option><option value=\"3001\">判断题</option></select></td>" +
			...
			"<button type=\"button\" class=\"btn btn-default\" style=\"float: right; margin-right: 50px;\" onclick=\"addFillBlank()()\">确认</button>");
	e.stopPropagation();
});
...
$(document).on("click",".table_input_modify",function(e){
	...
	var fixed = $(this).parent().parent().children().first().next().next().next().next().next().find("td").html();
	var ans = $(this).parent().parent().children().first().next().next().next().next().next().next().find("td").html();
	...
	if(typeof(fixed) == "undefined"){
		fixed = $(this).parent().parent().children().first().next().next().next().next().next().html();
	}
	if(typeof(ans) == "undefined"){
		ans = $(this).parent().parent().children().first().next().next().next().next().next().next().html();
	}
	if(fixed == "判断题"){
		if(ans == "正确"){
			ans = "<select class=\"form-control\"><option value=\"\">请选择</option><option value=\"6000\" selected>正确</option><option value=\"6001\">错误</option></select>";
		}else{
			ans = "<select class=\"form-control\"><option value=\"\">请选择</option><option value=\"6000\">正确</option><option value=\"6001\" selected>错误</option></select>";
		}
		
	}else{
		ans = "<input class=\"form-control\" value=\""+ans+"\">";
	}
	$('#table_input').html("");
	$('#table_input').css({'position':'absolute','top':'25%','left':'3px','width':'100%','z-index':'12'});
	$('#table_input').show();
...
function table_input_addButton(){
	...
	var fixed = x.find("td").next().next().next().next().next().find("select").val();
	var ans = x.find("td").next().next().next().next().next().next().find("input").val();
	if(typeof(ans) == "undefined"){
		ans = x.find("td").next().next().next().next().next().next().find("select").val();
	}
	if(chapter == "" || chapter == null){
		...
		swal("题目为空!");
		return false;
	}else if( parseInt(fixed) != 3000 && parseInt(fixed) != 3001){
		swal("题型为空!");
		return false;
...
function addFillBlank(){
	...
	for(var i = 0; i < length; i++){
		...
		fixed = x.find("td").next().next().next().next().next().find("select").val();
		ans = x.find("td").next().next().next().next().next().next().find("input").val();
		if(typeof(ans) == "undefined"){
			ans = x.find("td").next().next().next().next().next().next().find("select").val();
		}
		...
		addJosnData += courseCode+"`"+chapterCode+"`"+level+"`"+title+"`"+fixed+"`"+ans+"`";
		var xx = x.next();
		x = xx;
	}
	$.ajax({
		type : "post",
		url : "../fillblank/addfillblank",
...
$(document).on("click",".searchCourseSelected",function(e){
	var course = $(this).html();
	courseCodeForAll = $(this).next().html();
	var x = $("#fillblank_course").parent().children().last();
	if(typeof(x.attr("class")) == "undefined"){
		x.remove();
	}
	$("#fillblank_course").val(course);
	$("#fillblank_course").parent().append("<span style=\"display:none;\">"+courseCodeForAll+"</span>");
	$("#course_chapterSelectArea").hide();
	e.stopPropagation();
});
